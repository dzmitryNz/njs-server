...
const getById = async (strIngredient: string): Promise<ItemType | undefined> => {
  const list = await readItemList();

  return list.find((v) => {
    return v.strIngredient === strIngredient;
  });
};

const getByCat = async (strCategory: string): Promise<ItemType | undefined> => {
  const list = await readItemList();

  return list.find((v) => {
    return v.strCategory === strCategory;
  });
};
